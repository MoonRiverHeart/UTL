...
|------|------|
| 单击节点 | 选中节点 |
| 双击节点 | 快速编辑节点名称（inline编辑） |
| 拖拽节点 | 按住节点拖动移动位置（自由布局模式） |
| 右侧蓝色圆点 | 拖拽创建连线，默认"包含"关系 |
| 点击连线徽章 | 编辑关系类型或删除连线 |
| 快速添加节点 | 工具栏下拉菜单，选择类型直接创建 |
| 删除节点 | 选中后点击删除按钮，或节点树右键删除 |
| 布局模式切换 | 自由布局（可拖动）/ 约束布局（自动排列） |
...
|------|------|
| 自由布局 | 节点可自由拖动到任意位置 |
| 约束布局 | 节点位置由系统自动计算，层级树状结构，紧凑排列，删除节点后自动重新布局 |
...
| 接口 | 说明 |
|------|------|
| `GET /api/workspaces` | 工作区列表（含角色信息） |
| `POST /api/workspaces` | 创建工作区 |
| `GET /api/workspaces/:id` | 工作区详情 |
| `PUT /api/workspaces/:id` | 更新工作区名称（仅所有者） |
| `DELETE /api/workspaces/:id` | 删除工作区（仅所有者） |
| `GET /api/workspaces/:id/collaborators` | 获取协作者列表 |
| `POST /api/workspaces/:id/collaborators` | 邀请成员（仅所有者） |
| `PUT /api/workspaces/:id/collaborators/:cid` | 更新成员角色（仅所有者） |
| `DELETE /api/workspaces/:id/collaborators/:cid` | 移除成员（仅所有者） |
...
// editorStore - 编辑器状态
{ mode, nodes, relations, selectedNodes, layoutMode, setNodes, selectNode, setLayoutMode }
```
